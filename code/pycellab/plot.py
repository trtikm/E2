...
def curve(points, pathname, title=None, xaxis_name=None, faxis_name=None):
    __write_xplot(lambda ax, x, y: ax.plot(x, y), points, pathname, title, xaxis_name, faxis_name)


def scatter(points, pathname, title=None, xaxis_name=None, faxis_name=None):
    if not title:
        title = "#points=" + str(len(points))
    __write_xplot(lambda ax, x, y: ax.scatter(x, y, s=2), points, pathname, title, xaxis_name, faxis_name)


def histogram(distrib, pathname, normalised=True, markers="o", title=None, xaxis_name=None, faxis_name=None):
    ...
    __write_xplot(
        lambda ax, x, y: ax.stem(x, y, markerfmt=markers, basefmt="None"),
