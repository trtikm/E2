#include <gfx/shader_data_bindings.hpp>
#include <utility/invariants.hpp>
#include <utility/assumptions.hpp>
#include <unordered_map>

namespace gfx {


std::string  name(VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION const  location)
{
    switch (location)
    {
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_POSITION : return "IN_POSITION";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_DIFFUSE  : return "IN_DIFFUSE";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_SPECULAR : return "IN_SPECULAR";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_NORMAL   : return "IN_NORMAL";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_TANGENT  : return "IN_TANGENT";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_BITANGENT: return "IN_BITANGENT";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_INDICES_OF_MATRICES: return "IN_INDICES_OF_MATRICES";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_WEIGHTS_OF_MATRICES: return "IN_WEIGHTS_OF_MATRICES";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_TEXCOORD0: return "IN_TEXCOORD0";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_TEXCOORD1: return "IN_TEXCOORD1";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_INSTANCED_MATRIX_FROM_MODEL_TO_CAMERA: return "IN_INSTANCED_MATRIX_FROM_MODEL_TO_CAMERA";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_INSTANCED_DIFFUSE_COLOUR: return "IN_INSTANCED_DIFFUSE_COLOUR";
    default: UNREACHABLE();
    }
}

std::string  type_name(VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION const  location)
{
    switch (location)
    {
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_POSITION : return "vec3";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_DIFFUSE  : return "vec4";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_SPECULAR : return "vec4";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_NORMAL   : return "vec3";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_TANGENT  : return "vec3";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_BITANGENT: return "vec3";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_INDICES_OF_MATRICES: return "ivec4";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_WEIGHTS_OF_MATRICES: return "vec4";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_TEXCOORD0: return "vec2";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_TEXCOORD1: return "vec2";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_INSTANCED_MATRIX_FROM_MODEL_TO_CAMERA: return "mat4";
    case VERTEX_SHADER_INPUT_BUFFER_BINDING_LOCATION::IN_INSTANCED_DIFFUSE_COLOUR: return "vec4";
    default: UNREACHABLE();
    }
}


std::string  name(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION const  location)
{
    switch (location)
    {
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_POSITION : return "PASS_POSITION";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_DIFFUSE  : return "PASS_DIFFUSE";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_SPECULAR : return "PASS_SPECULAR";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_NORMAL   : return "PASS_NORMAL";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_TANGENT  : return "PASS_TANGENT";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_BITANGENT: return "PASS_BITANGENT";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD0: return "PASS_TEXCOORD0";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD1: return "PASS_TEXCOORD1";
    default: UNREACHABLE();
    }
}

std::string  type_name(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION const  location)
{
    switch (location)
    {
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_POSITION : return "vec3";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_DIFFUSE  : return "vec4";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_SPECULAR : return "vec4";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_NORMAL   : return "vec3";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_TANGENT  : return "vec3";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_BITANGENT: return "vec3";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD0: return "vec2";
    case VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD1: return "vec2";
    default: UNREACHABLE();
    }
}


std::string  name(VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME const  symbolic_name)
{
    switch (symbolic_name)
    {
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::MATRIX_FROM_MODEL_TO_CAMERA: return "MATRIX_FROM_MODEL_TO_CAMERA";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::MATRIX_FROM_CAMERA_TO_CLIPSPACE: return "MATRIX_FROM_CAMERA_TO_CLIPSPACE";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::MATRICES_FROM_MODEL_TO_CAMERA: return "MATRICES_FROM_MODEL_TO_CAMERA";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::NUM_MATRICES_PER_VERTEX: return "NUM_MATRICES_PER_VERTEX";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::AMBIENT_COLOUR: return "AMBIENT_COLOUR";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::DIFFUSE_COLOUR: return "DIFFUSE_COLOUR";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::SPECULAR_COLOUR: return "SPECULAR_COLOUR";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_DIRECTION: return "DIRECTIONAL_LIGHT_DIRECTION";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_COLOUR: return "DIRECTIONAL_LIGHT_COLOUR";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_COLOUR: return "FOG_COLOUR";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_NEAR: return "FOG_NEAR";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_FAR: return "FOG_FAR";
    default: UNREACHABLE();
    }
}

std::string  type_name(VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME const  symbolic_name)
{
    switch (symbolic_name)
    {
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::MATRIX_FROM_MODEL_TO_CAMERA: return "mat4";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::MATRIX_FROM_CAMERA_TO_CLIPSPACE: return "mat4";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::MATRICES_FROM_MODEL_TO_CAMERA: return "mat4";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::NUM_MATRICES_PER_VERTEX: return "uint";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::AMBIENT_COLOUR: return "vec3";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::DIFFUSE_COLOUR: return "vec4";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::SPECULAR_COLOUR: return "vec4";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_DIRECTION: return "vec3";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_COLOUR: return "vec3";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_COLOUR: return "vec4";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_NEAR: return "float";
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_FAR: return "float";
    default: UNREACHABLE();
    }
}

natural_32_bit  num_elements(VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME const  symbolic_name)
{
    switch (symbolic_name)
    {
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::MATRIX_FROM_MODEL_TO_CAMERA: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::MATRIX_FROM_CAMERA_TO_CLIPSPACE: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::MATRICES_FROM_MODEL_TO_CAMERA: return 128;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::NUM_MATRICES_PER_VERTEX: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::AMBIENT_COLOUR: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::DIFFUSE_COLOUR: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::SPECULAR_COLOUR: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_DIRECTION: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_COLOUR: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_COLOUR: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_NEAR: return 1U;
    case VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_FAR: return 1U;
    default: UNREACHABLE();
    }
}

VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME  to_symbolic_uniform_name_of_vertex_shader(std::string  name)
{
    static std::unordered_map<std::string, VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME> const  map =
        []() -> std::unordered_map<std::string, VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME> {
            std::unordered_map<std::string, VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME> map;
            for (natural_32_bit i = 0U; i < num_VERTEX_SHADER_UNIFORM_SYMBOLIC_NAMEs(); ++i)
                map.insert({
                    gfx::name(static_cast<VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME>(i)),
                    static_cast<VERTEX_SHADER_UNIFORM_SYMBOLIC_NAME>(i)
                    });
            return map;
        }();

    auto const  it = map.find(name);
    ASSUMPTION(it != map.cend());
    return it->second;
}


std::string  name(FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION const  location)
{
    switch (location)
    {
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_POSITION : return "PASS_POSITION";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_DIFFUSE  : return "PASS_DIFFUSE";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_SPECULAR : return "PASS_SPECULAR";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_NORMAL   : return "PASS_NORMAL";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_TANGENT  : return "PASS_TANGENT";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_BITANGENT: return "PASS_BITANGENT";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD0: return "PASS_TEXCOORD0";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD1: return "PASS_TEXCOORD1";
    default: UNREACHABLE();
    }
}

std::string  type_name(FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION const  location)
{
    switch (location)
    {
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_POSITION : return "vec3";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_DIFFUSE  : return "vec4";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_SPECULAR : return "vec4";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_NORMAL   : return "vec3";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_TANGENT  : return "vec3";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_BITANGENT: return "vec3";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD0: return "vec2";
    case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD1: return "vec2";
    default: UNREACHABLE();
    }
}


std::string  name(FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION const  location)
{
    switch (location)
    {
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_COLOUR: return "OUT_COLOUR";
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_TEXTURE_POSITION: return "OUT_TEXTURE_POSITION";
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_TEXTURE_NORMAL: return "OUT_TEXTURE_NORMAL";
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_TEXTURE_DIFFUSE: return "OUT_TEXTURE_DIFFUSE";
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_TEXTURE_SPECULAR: return "OUT_TEXTURE_SPECULAR";
    default: UNREACHABLE();
    }
}

std::string  type_name(FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION const  location)
{
    switch (location)
    {
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_COLOUR: return "vec4";
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_TEXTURE_POSITION: return "vec3";
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_TEXTURE_NORMAL: return "vec3";
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_TEXTURE_DIFFUSE: return "vec4";
    case FRAGMENT_SHADER_OUTPUT_BINDING_LOCATION::OUT_TEXTURE_SPECULAR: return "vec4";
    default: UNREACHABLE();
    }
}


std::string  name(FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME const  uniform_symbolic_name)
{
    switch (uniform_symbolic_name)
    {
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::TEXTURE_SAMPLER_DIFFUSE: return "TEXTURE_SAMPLER_DIFFUSE";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::TEXTURE_SAMPLER_SPECULAR: return "TEXTURE_SAMPLER_SPECULAR";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::TEXTURE_SAMPLER_NORMAL: return "TEXTURE_SAMPLER_NORMAL";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::AMBIENT_COLOUR: return "AMBIENT_COLOUR";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::DIFFUSE_COLOUR: return "DIFFUSE_COLOUR";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::SPECULAR_COLOUR: return "SPECULAR_COLOUR";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_DIRECTION: return "DIRECTIONAL_LIGHT_DIRECTION";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_COLOUR: return "DIRECTIONAL_LIGHT_COLOUR";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_COLOUR: return "FOG_COLOUR";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_NEAR: return "FOG_NEAR";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_FAR: return "FOG_FAR";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::ALPHA_TEST_CONSTANT: return "ALPHA_TEST_CONSTANT";
    default: UNREACHABLE();
    }
}

std::string  type_name(FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME const  uniform_symbolic_name)
{
    switch (uniform_symbolic_name)
    {
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::TEXTURE_SAMPLER_DIFFUSE: return "sampler2D";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::TEXTURE_SAMPLER_SPECULAR: return "sampler2D";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::TEXTURE_SAMPLER_NORMAL: return "sampler2D";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::AMBIENT_COLOUR: return "vec3";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::DIFFUSE_COLOUR: return "vec4";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::SPECULAR_COLOUR: return "vec4";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_DIRECTION: return "vec3";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::DIRECTIONAL_LIGHT_COLOUR: return "vec3";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_COLOUR: return "vec4";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_NEAR: return "float";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::FOG_FAR: return "float";
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::ALPHA_TEST_CONSTANT: return "float";
    default: UNREACHABLE();
    }
}

natural_32_bit  num_elements(FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME const  symbolic_name)
{
    return 1U;
}

bool  is_texture_sampler(FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME const  uniform_symbolic_name)
{
    switch (uniform_symbolic_name)
    {
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::TEXTURE_SAMPLER_DIFFUSE:
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::TEXTURE_SAMPLER_SPECULAR:
    case FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME::TEXTURE_SAMPLER_NORMAL:
        return true;
    default:
        return false;
    }
}

FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME  to_symbolic_uniform_name_of_fragment_shader(std::string  name)
{
    static std::unordered_map<std::string, FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME> const  map =
        []() -> std::unordered_map<std::string, FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME> {
            std::unordered_map<std::string, FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME> map;
            for (natural_32_bit i = 0U; i < num_FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAMEs(); ++i)
                map.insert({
                    gfx::name(static_cast<FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME>(i)),
                    static_cast<FRAGMENT_SHADER_UNIFORM_SYMBOLIC_NAME>(i)
                    });
            return map;
        }();

    auto const  it = map.find(name);
    ASSUMPTION(it != map.cend());
    return it->second;
}


bool  compatible(std::unordered_set<VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION> const& vertex_program_output,
                 std::unordered_set<FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION> const& fragment_program_input)
{
    for (auto const  input : fragment_program_input)
        switch (input)
        {
        case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_POSITION :
            if (vertex_program_output.count(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_POSITION) == 0ULL)
                return false;
            break;
        case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_DIFFUSE  :
            if (vertex_program_output.count(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_DIFFUSE) == 0ULL)
                return false;
            break;
        case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_SPECULAR :
            if (vertex_program_output.count(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_SPECULAR) == 0ULL)
                return false;
            break;
        case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_NORMAL   :
            if (vertex_program_output.count(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_NORMAL) == 0ULL)
                return false;
            break;
        case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_TANGENT:
            if (vertex_program_output.count(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_TANGENT) == 0ULL)
                return false;
            break;
        case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_BITANGENT:
            if (vertex_program_output.count(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_BITANGENT) == 0ULL)
                return false;
            break;
        case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD0:
            if (vertex_program_output.count(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD0) == 0ULL)
                return false;
            break;
        case FRAGMENT_SHADER_INPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD1:
            if (vertex_program_output.count(VERTEX_SHADER_OUTPUT_BUFFER_BINDING_LOCATION::PASS_TEXCOORD1) == 0ULL)
                return false;
            break;
        default:
            UNREACHABLE();
        }
    return true;
}


}
